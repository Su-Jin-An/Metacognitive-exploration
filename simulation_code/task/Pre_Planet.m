function [mouse_event] = Pre_Planet(dstnt,deg)
%% Global parameters
...
Rwd_ang_rad = deg; % rad
PlanetColor = [000000]; % [1-deg./pi, deg./pi, 0];
...
plotDist(Dist_grid_pos,Dist_grid_neg,Dist_size,'Pre');
...
while ~end_flag % Until end_flag is setting
    ...
    if(etime(clock,t0) > 40) % After 40sec, diplay 'No Fuel' Message
        NoFuel(planetRect,dstnt,deg,PreTimeLimit);
        end_flag = 1;
    end
    if (buttons == 1) && (etime(clock,t0) - last_clk > 0.25) % if Mouse clicked, refractory period is 0.25 sec.
    ...
    String1 = double('행성 안을 클릭해 행성의 특성에 대해 배우세요.');
